...
date: "2026-04-26"
source: "MEMORY.md rotation; Desktop 20260713_174939_5f4b63 → 20260714_142753_b2cf3c; WA 20260715_063318"
...
> **2026-07:** el pod **se recrea on-demand** (skill `comfy-ui` / `runpod` / `pod.py`). No asumir IP/Pod ID fijos. H200 ~$4.39/h. **Shutdown split (15/7 WA):** si Jorge pidió el start → al terminar **preguntar** si apaga; si Brisa prendió sola (contenido/influencer) → `stop --force` **sola**. Cron social no auto-start H200 salvo que no quede ningún clip SFW usable.

- **Name (scripts/API, rota):** hist. `brisa-comfyui-h200` → `comfyui-luz-h200` → visto **`comfyui-luz-2`** el 20/7. No hardcodear nombre.
- **Pod ID reciente (puede cambiar):** `1n0whm7wacau00` (13–17/7 h200) · **`847sa5pd4i5pkm`** (`comfyui-luz-2`, 20/7). Siempre `pod.py status`.
- **GPU:** H200 143GB VRAM
- **SSH:** host y **puerto público cambian en cada start**. Key local `~/.ssh/id_rsa`; en pod a veces también template key. Usar `StrictHostKeyChecking=no` si rota host key.
...
- **Autostart:** Template `t9hgwtx2xb` → ComfyUI + nginx + cloudflared (~2–4 min a 8188 listo). Si 530/no responde: arrancar Comfy por SSH/tmux (`python3 main.py --listen 0.0.0.0 --port 8188 --highvram` + `LD_LIBRARY_PATH` CUDA) y a veces `pip install sqlalchemy --break-system-packages`.
- **2026-07-15:** batch erótico madrugada + SFW breakfast/kitchen de día (WA) → stop → EXITED.
- **2026-07-20 WA:** pod **`comfyui-luz-2` EXITED** (no factura). Jorge mandó ref foto café SFW; oferta video lifestyle pendiente de OK (ops comercial Brisa = agencia; solo si Jorge pide explícito).
- **Train Wan:** apagar Comfy (OOM). Scripts I2V: `/workspace/scripts/bere/wan_i2v_api_builder.py`
- **Shutdown split:** si Jorge pidió start → preguntar al apagar; si Nina prendió sola → `stop --force` sola + verificar EXITED. Post-18/7 no hay cron social Brisa.

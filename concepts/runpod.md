...
- **GPU:** H200 143GB VRAM
- **SSH:** host `103.196.86.23`, **puerto público cambia en cada start** (ej. 10985 → 17264 → 18700 → 19210 → **18243** → **12131** el 15/7). Key local `~/.ssh/id_rsa`; en pod a veces también template key. Usar `StrictHostKeyChecking=no` si rota host key.
...
- **2026-07-15:** batch erótico madrugada + SFW breakfast/kitchen de día (WA) → `pod.py stop --force` → **EXITED**. Reanudar con OK Jorge si él pidió el start.
